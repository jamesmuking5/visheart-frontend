...
export interface DecodedMasks {
  aiMasks: Record<string, Uint8Array>;
  manualMasks: Record<string, Uint8Array>;
}
 ...
 * 
 * @param aiMasks - Array of AI segmentation masks (isMedSAMOutput: true)
 * @param manualMasks - Array of manual segmentation masks (isMedSAMOutput: false)
 * @param projectDimensions - Project dimensions (width and height)
 * @returns DecodedMasks object containing decoded AI and manual masks
 */
export function decodeSegmentationMasks(
  ...
  const decodedAiMasks: Record<string, Uint8Array> = {};
  const decodedManualMasks: Record<string, Uint8Array> = {};

  const { width, height } = projectDimensions;

  // Decode AI masks
  aiMasks.forEach((maskData, maskIndex) => {
    if (maskData.frames) {
      maskData.frames.forEach((frame: any, frameIndex: number) => {
        if (frame.slices) {
          frame.slices.forEach((slice: any, sliceIndex: number) => {
            if (slice.segmentationmasks) {
              slice.segmentationmasks.forEach((mask: any) => {
                const maskKey = `ai_mask_${maskIndex}_frame_${frameIndex}_slice_${sliceIndex}_${mask.class}`;
                const decodedMask = rleDecodeToArray(
                  mask.segmentationmaskcontents,
                  height,
                  width,
                );
                decodedAiMasks[maskKey] = decodedMask;
              });
            }
          });
        }
  ...
  // Decode manual masks
  manualMasks.forEach((maskData, maskIndex) => {
    if (maskData.frames) {
      maskData.frames.forEach((frame: any, frameIndex: number) => {
        if (frame.slices) {
          frame.slices.forEach((slice: any, sliceIndex: number) => {
            if (slice.segmentationmasks) {
              slice.segmentationmasks.forEach((mask: any) => {
                const maskKey = `manual_mask_${maskIndex}_frame_${frameIndex}_slice_${sliceIndex}_${mask.class}`;
                const decodedMask = rleDecodeToArray(
                  mask.segmentationmaskcontents,
                  height,
                  width,
                );
                decodedManualMasks[maskKey] = decodedMask;
              });
            }
          });
        }
  ...
  console.log(
    `Decoded ${Object.keys(decodedAiMasks).length} AI masks and ${Object.keys(decodedManualMasks).length} manual masks`,
  );

  return {
    aiMasks: decodedAiMasks,
    manualMasks: decodedManualMasks,
  };
}
